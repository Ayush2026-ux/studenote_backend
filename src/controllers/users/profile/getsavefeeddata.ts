...
export const getSaveFeedData = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const savedFeeds = await saveModule
      ...
      .populate({
        ...
        populate: [
          ...
          },
          {
            path: "note",
            select:
              "_id title course subject semester price thumbnail file views downloads pages description",
          },
        ],
      })
      .lean();
    ...
    const feeds = savedFeeds
      .filter((sf: any) => sf.feed && sf.feed.note)
      .map((sf: any) => {
        const feed = sf.feed;
